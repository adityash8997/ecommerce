...
import { Upload, Printer, TruckIcon, DollarSign, AlertTriangle, User, Shield } from 'lucide-react';
...
  const { user } = useAuth();
  // FIX 1: Destructure createPrintJob from the usePrintJobManager hook
  const { createPrintJob, isLoading } = usePrintJobManager();
...
    
    if (success) {
      // FIX 2: Corrected the incomplete setTimeout syntax
      setTimeout(() => {
        // Reset form
        setFormData({
          studentName: '',
...
          </div>
          
          {/* FIX 3: Removed redundant debug tools section */}
          {user && (
            <div className="mb-8">
              <PrintJobTester />
            </div>
...
                    <p className="text-muted-foreground mb-4">Please sign in to view your print orders</p>
                    {/* FIX 4: Changed to use navigate for better SPA experience */}
