...
const BuyPrelovedBooks = () => {
  ...
  useEffect(() => {
    console.log('BuyPrelovedBooks component mounted');
    console.log('Current filters:', filters);
    loadBooks();
  }, [filters]);

  const loadBooks = async () => {
    console.log('loadBooks called with filters:', filters);
    setIsLoading(true);
    try {
                    ...
                    <CardContent className="p-6">
                      <div className="flex justify-between items-start mb-4">
                        <Badge variant="outline" className="text-xs">
                          Semester {book.semester_book?.semester || 'N/A'}
                        </Badge>
                        <div className="text-right">
                          <div className="text-sm text-muted-foreground">{conditionInfo?.emoji || '📖'}</div>
                          <div className="text-xs text-muted-foreground">{conditionInfo?.label || 'Good'}</div>
                        </div>
                      </div>
                      
                      <h3 className="font-semibold text-lg mb-2 line-clamp-2 group-hover:text-kiit-green transition-colors">
                        {book.semester_book?.book_name || 'Unknown Book'}
                      </h3>
                      
                      <div className="space-y-1 text-sm text-muted-foreground mb-4">
                        <p><span className="font-medium">Author:</span> {book.semester_book?.author || 'Unknown'}</p>
                        <p><span className="font-medium">Edition:</span> {book.semester_book?.edition || 'N/A'}</p>
                        <p><span className="font-medium">Publisher:</span> {book.semester_book?.publisher || 'N/A'}</p>
                      </div>
