...
CREATE POLICY "Workers can view their own profile" ON public.book_workers 
FOR SELECT USING (auth.uid()::text IN (SELECT user_id::text FROM profiles WHERE email = book_workers.email));

CREATE POLICY "Admins and workers can view verifications" ON public.book_verifications 
FOR SELECT USING (
  get_current_user_role() = 'admin'::text OR 
  worker_id IN (SELECT id FROM book_workers WHERE email IN (SELECT email FROM profiles WHERE id = auth.uid()))
);
